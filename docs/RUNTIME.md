...
`hflow down`, edit `API_PORT`, `hflow up` again. The API binds to
`127.0.0.1` by default (`API_BIND_HOST` in `.env` widens it deliberately),
so two bundles on different ports coexist fine.
