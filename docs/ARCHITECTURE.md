...
The one measured bottleneck is per-message decoding of state channels in pure Python (~38k msgs/s): roughly seconds per episode against minutes of H.264 encode, i.e. not v1's constraint. The mitigation is architectural now, native later: video bytes are passed opaque (remuxed by ffmpeg, never message-decoded in Python), and the episode reader is a **batch-oriented interface** (decoded batches in, Arrow/bytes out) so a Rust-backed reader (PyO3 bindings over the official [`mcap` Rust crate](https://docs.rs/mcap), shipped as a separate wheel-distributing package with the pure-Python backend as its parity-tested fallback) can drop in behind the same interface as a non-blocking parallel track. Nothing in the pipeline waits on it. The one API rule that cannot be retrofitted is the interface shape: batch iterators, never per-message callbacks in the hot loop.
...
A benchmark report (tracked in issues) will publish what the simple version achieves and state honestly where each ceiling sits: storage vs per-frame JPEG (Dyna: ~68% reduction), topic-group vs default chunking read performance (Dyna: ~2.9× faster), and single-machine ingestion throughput.
...
- [MCAP specification](https://mcap.dev/spec); [MCAP Python libraries](https://mcap.dev/docs/python); [`mcap` Rust crate](https://docs.rs/mcap); [MCAP CLI](https://mcap.dev/guides/cli) (`doctor`, `recover`, `filter`)
...
Robot-learning data ecosystem:

- [LeRobotDataset v3](https://huggingface.co/blog/lerobot-datasets-v3); [LeRobot](https://github.com/huggingface/lerobot)
- [openpi](https://github.com/Physical-Intelligence/openpi) (Physical Intelligence)
- [NVIDIA Isaac GR00T: MCAP-to-LeRobot export](https://docs.nvidia.com/learning/physical-ai/gr00t-e2e-workflow/latest/real-robot-workflow/real-data-export.html)
- [Trossen Robotics: MCAP vs LeRobot vs OpenPi](https://www.trossenrobotics.com/post/robotics-data-pipeline-tools-mcap-vs-lerobot-vs-openpi)
- [Open X-Embodiment](https://github.com/google-deepmind/open_x_embodiment) (RLDS); [DROID](https://droid-dataset.github.io/)
- [forge](https://github.com/arpitg1304/forge) (multi-format robotics dataset converter); [kappe](https://github.com/sensmore/kappe) (MCAP processing)

Quality control and curation research:

- robomimic, [What Matters in Learning from Offline Human Demonstrations](https://robomimic.github.io/study)
- Voxel51, [Robot data quality scoring](https://voxel51.com/blog/robot-data-quality-scoring)
- RobotData, [One-click robot data curation](https://huggingface.co/blog/robotdata/score-lerobot-episodes)
- Scizor: [Self-Supervised Data Curation for Large-Scale Imitation Learning](https://arxiv.org/abs/2505.22626)
- DemInf: [state-action mutual information curation](https://www.roboticsproceedings.org/rss21/p023.pdf) (RSS 2025)
- Siemens DQAF ([arXiv:2605.26349](https://arxiv.org/abs/2605.26349)); SARM ([arXiv:2509.25358](https://arxiv.org/abs/2509.25358))
- [1X Redwood](https://www.1x.tech/discover/redwood-ai) (training on failures); RoboMIND ([arXiv:2412.13877](https://arxiv.org/abs/2412.13877))

Production-experience grounding:

- Pareto: Hebbian Robotics' robotics data-quality system; source of the durability idioms, the single-pass ffmpeg instrument design, the pinned-ffmpeg discipline, and the failure taxonomy adopted here
